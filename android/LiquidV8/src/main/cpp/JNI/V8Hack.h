...
public:
    ...
    static void PostTaskWithInterrupt(V8Task* task) {
        Isolate* isolate = task->taskRunner->_isolate;
        isolate->RequestInterrupt(&PostTaskCallback, task);
    }

    static void PostTaskCallback(Isolate* isolate, void* data) {
        V8Task* task = static_cast<V8Task*>(data);
        task->taskRunner->PostTask(std::move(task->task));
        delete task;
    }
...
//        return mPlatform->GetForegroundTaskRunner(isolate);
//    }
//    virtual std::shared_ptr<TaskRunner> GetBackgroundTaskRunner(Isolate* isolate) override {
//        return mPlatform->GetBackgroundTaskRunner(isolate);
//    }
...
//            V8HackedTaskRunner* tr = new V8HackedTaskRunner(isolate, mPlatform->GetWorkerThreadsTaskRunner(isolate), mQueueTask);
